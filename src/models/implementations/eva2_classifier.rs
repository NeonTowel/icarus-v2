...
fn construct_2d_rope(
    grid_size: usize,
    dim: usize,       // 64
    temperature: f64, // 10000.0
    device: &Device,
) -> Result<Tensor> {
    let num_bands = dim / 4; // 16
    let mut bands = Vec::with_capacity(num_bands);
    for i in 0..num_bands {
        let exp = (i as f64) / (num_bands as f64);
        bands.push(1.0 / temperature.powf(exp));
    }

    let mut sin_emb = Vec::with_capacity(grid_size * grid_size * dim);
    let mut cos_emb = Vec::with_capacity(grid_size * grid_size * dim);

    for h in 0..grid_size {
        for w in 0..grid_size {
            for &coord in &[h as f64, w as f64] {
                for &band in &bands {
                    let pos = coord * band;
                    let s = pos.sin() as f32;
                    let c = pos.cos() as f32;
                    sin_emb.push(s);
                    sin_emb.push(s);
                    cos_emb.push(c);
                    cos_emb.push(c);
    ...
    let sin_tensor = Tensor::from_vec(sin_emb, (grid_size * grid_size, dim), device)?;
    let cos_tensor = Tensor::from_vec(cos_emb, (grid_size * grid_size, dim), device)?;

    Tensor::cat(&[&sin_tensor, &cos_tensor], 1)
}
...
impl EVA2VisionTransformer {
    pub fn new(vb: VarBuilder, depth: usize, embed_dim: usize, num_heads: usize) -> Result<Self> {
        ...
        let rot_pos_embed =
            construct_2d_rope(IMG_SIZE / PATCH_SIZE, head_dim, 10000.0, vb.device())?;

        let head = linear(vb.pp("head"), embed_dim, NUM_CLASSES, true)?;
        let norm = layer_norm(embed_dim, 1e-6, vb.pp("fc_norm"))?;
...
impl ImageClassifier for EVA2VisionTransformer {
    fn classify(&self, image: &image::DynamicImage) -> anyhow::Result<u8> {
        let device = self.cls_token.device();

        // 1. Resize to 448x448 using Bicubic (CatmullRom) interpolation
        ...
        // 2. Convert to RGB and then to a raw byte buffer
        let rgb = resized.to_rgb8();
        let data = rgb.into_raw();

        // 3. Create tensor
        let tensor = Tensor::from_vec(data, (IMG_SIZE, IMG_SIZE, 3), device)?
            .permute((2, 0, 1))?
            .to_dtype(candle_core::DType::F32)?
            .affine(1.0 / 255.0, 0.0)?;

        // 4. Normalize
        let mean = Tensor::new(&[0.481f32, 0.458f32, 0.408f32], device)?.reshape((3, 1, 1))?;
        let std = Tensor::new(&[0.268f32, 0.261f32, 0.275f32], device)?.reshape((3, 1, 1))?;
        let tensor = tensor
            .broadcast_sub(&mean)?
            .broadcast_div(&std)?
            .unsqueeze(0)?;
        ...
        // 5. Forward pass
        let logits = candle_nn::Module::forward(self, &tensor)?;

        // 6. Argmax to get tier
        let logits_vec: Vec<f32> = logits.squeeze(0)?.to_vec1()?;
        Ok(logits_to_tier(&logits_vec))
    }
...
/// Map a logits vector to an Icarus tier (1-4) by taking the argmax index + 1.
/// Freepik class order: 0=neutral, 1=low, 2=medium, 3=high.
fn logits_to_tier(logits: &[f32]) -> u8 {
...
pub struct FreepikEva02 {
    model: EVA2VisionTransformer,
}

impl FreepikEva02 {
    pub fn from_hub(device: &candle_core::Device) -> anyhow::Result<Self> {
        let api = hf_hub::api::sync::Api::new()?;
        let repo = api.model("Freepik/nsfw_image_detector".to_string());
        ...
        // Freepik uses EVA-02 base (768 dim, 12 heads, 12 depth)
        let model = vit_base(vb)?;
        Ok(Self { model })
    }
}

impl ImageClassifier for FreepikEva02 {
    fn classify(&self, image: &image::DynamicImage) -> anyhow::Result<u8> {
        self.model.classify(image)
    }
...
#[cfg(test)]
mod tests {
    use super::logits_to_tier;

    #[test]
    fn test_logit_to_tier_neutral() {
        assert_eq!(logits_to_tier(&[10.0, 1.0, 2.0, 3.0]), 1);
    }

    #[test]
    fn test_logit_to_tier_low() {
        assert_eq!(logits_to_tier(&[1.0, 10.0, 2.0, 3.0]), 2);
    }

    #[test]
    fn test_logit_to_tier_medium() {
        assert_eq!(logits_to_tier(&[1.0, 2.0, 10.0, 3.0]), 3);
    }

    #[test]
    fn test_logit_to_tier_high() {
        assert_eq!(logits_to_tier(&[1.0, 2.0, 3.0, 10.0]), 4);
    }

    #[test]
    fn test_logit_to_tier_first_wins_on_tie() {
        // When values are equal, first index wins (stable argmax)
        assert_eq!(logits_to_tier(&[5.0, 5.0, 5.0, 5.0]), 1);
    }
}
